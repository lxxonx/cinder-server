...
// QueryFriendsReq chains the current query on the "friends_req" edge.
...
func (uq *UserQuery) QueryLikeTo() *GroupQuery {
...
// QuerySave chains the current query on the "save" edge.
func (uq *UserQuery) QuerySave() *GroupQuery {
	query := &GroupQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) QueryChatroom() *ChatRoomQuery {
	query := &ChatRoomQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryMessage() *ChatMessageQuery {
	query := &ChatMessageQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// the "friends_req" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) WithLikeTo(opts ...func(*GroupQuery)) *UserQuery {
	query := &GroupQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// WithSave tells the query-builder to eager-load the nodes that are connected to
// the "save" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSave(opts ...func(*GroupQuery)) *UserQuery {
	query := &GroupQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithChatroom(opts ...func(*ChatRoomQuery)) *UserQuery {
	query := &ChatRoomQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withChatroom = query
	return uq
}
...
func (uq *UserQuery) WithMessage(opts ...func(*ChatMessageQuery)) *UserQuery {
	query := &ChatMessageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withFriendsReq; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "friends_req": %w`, err)
		}
		query.Where(user.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "friends_req" node returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := uq.withSave; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "save": %w`, err)
		}
		query.Where(group.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "save" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Save = append(nodes[i].Edges.Save, n)
			}
		}
	}
	...
	if query := uq.withChatroom; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "chatroom": %w`, err)
		}
		query.Where(chatroom.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "chatroom" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Chatroom = append(nodes[i].Edges.Chatroom, n)
			}
		}
	}
	...
	if query := uq.withMessage; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Message = []*ChatMessage{}
		}
