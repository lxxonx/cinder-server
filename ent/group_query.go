...
// QueryLikeFromUser chains the current query on the "like_from_user" edge.
func (gq *GroupQuery) QueryLikeFromUser() *UserQuery {
	query := &UserQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QuerySaved chains the current query on the "saved" edge.
func (gq *GroupQuery) QuerySaved() *UserQuery {
	query := &UserQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryLikeFromGroup chains the current query on the "like_from_group" edge.
func (gq *GroupQuery) QueryLikeFromGroup() *GroupQuery {
...
// QueryLikeTo chains the current query on the "like_to" edge.
func (gq *GroupQuery) QueryLikeTo() *GroupQuery {
	query := &GroupQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withLikeFromUser:  gq.withLikeFromUser.Clone(),
...
// WithLikeFromUser tells the query-builder to eager-load the nodes that are connected to
// the "like_from_user" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithLikeFromUser(opts ...func(*UserQuery)) *GroupQuery {
	query := &UserQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withLikeFromUser = query
	return gq
}

// WithSaved tells the query-builder to eager-load the nodes that are connected to
// the "saved" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithSaved(opts ...func(*UserQuery)) *GroupQuery {
	query := &UserQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// WithLikeFromGroup tells the query-builder to eager-load the nodes that are connected to
// the "like_from_group" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithLikeFromGroup(opts ...func(*GroupQuery)) *GroupQuery {
	query := &GroupQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withLikeFromGroup = query
	return gq
}

// WithLikeTo tells the query-builder to eager-load the nodes that are connected to
// the "like_to" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithLikeTo(opts ...func(*GroupQuery)) *GroupQuery {
	query := &GroupQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if query := gq.withLikeFromUser; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, gq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "like_from_user": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "like_from_user" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.LikeFromUser = append(nodes[i].Edges.LikeFromUser, n)
			}
		}
	}
	...
	if query := gq.withSaved; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, gq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "saved": %w`, err)
		}
		query.Where(user.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "saved" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Saved = append(nodes[i].Edges.Saved, n)
			}
		}
	}
	...
	if query := gq.withLikeFromGroup; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, gq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "like_from_group": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "like_from_group" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.LikeFromGroup = append(nodes[i].Edges.LikeFromGroup, n)
			}
		}
	}
	...
	if query := gq.withLikeTo; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, gq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "like_to": %w`, err)
		}
		query.Where(group.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "like_to" node returned %v`, n.ID)
			}
			for i := range nodes {
